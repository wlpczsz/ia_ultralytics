...
from ia_ultralytics.hub.utils import HUB_API_ROOT, HUB_WEB_ROOT, PREFIX, smart_request
from ia_ultralytics.utils import LOGGER, __version__, checks, emojis, is_colab, threaded
from ia_ultralytics.utils.errors import HUBModelError
...
class HUBTrainingSession:
    """
    HUB training session for ia_ultralytics HUB YOLO models. Handles model initialization, heartbeats, and checkpointing.
    ...
    Attributes:
        ...
        model_url (str): URL for the model in ia_ultralytics HUB.
        api_url (str): API URL for the model in ia_ultralytics HUB.
        auth_header (dict): Authentication header for the ia_ultralytics HUB API requests.
        rate_limits (dict): Rate limits for different API calls (in seconds).
        timers (dict): Timers for rate limiting.
        metrics_queue (dict): Queue for the model's metrics.
        model (dict): Model data fetched from ia_ultralytics HUB.
    ...
    def upload_metrics(self):
        """Upload model metrics to ia_ultralytics HUB."""
        payload = {'metrics': self.metrics_queue.copy(), 'type': 'metrics'}
        smart_request('post', self.api_url, json=payload, headers=self.auth_header, code=2)

    def _get_model(self):
        """Fetch and return model data from ia_ultralytics HUB."""
    ...
    def upload_model(self, epoch, weights, is_best=False, map=0.0, final=False):
        """
        Upload a model checkpoint to ia_ultralytics HUB.
    ...
    def _start_heartbeat(self):
        """Begin a threaded heartbeat loop to report the agent's status to ia_ultralytics HUB."""
        while self.alive:
