...
from ia_ultralytics.utils import LOGGER, TQDM_BAR_FORMAT, checks, clean_url, emojis, is_online, url2file

# Define ia_ultralytics GitHub assets maintained at https://github.com/ia_ultralytics/assets
GITHUB_ASSETS_REPO = 'ia_ultralytics/assets'
...
def delete_dsstore(path, files_to_delete=('.DS_Store', '__MACOSX')):
    ...
    Example:
        ```python
        from ia_ultralytics.utils.downloads import delete_dsstore
...
def zip_directory(directory, compress=True, exclude=('.DS_Store', '__MACOSX'), progress=True):
    ...
    Example:
        ```python
        from ia_ultralytics.utils.downloads import zip_directory
...
def unzip_file(file, path=None, exclude=('.DS_Store', '__MACOSX'), exist_ok=False, progress=True):
    ...
    Example:
        ```python
        from ia_ultralytics.utils.downloads import unzip_file
...
def check_disk_space(url='https://ia_ultralytics.com/assets/coco128.zip', sf=1.5, hard=True):
    """
    Check if there is sufficient disk space to download and store a file.

    Args:
        url (str, optional): The URL to the file. Defaults to 'https://ia_ultralytics.com/assets/coco128.zip'.
...
def get_google_drive_file_info(link):
    ...
    Example:
        ```python
        from ia_ultralytics.utils.downloads import get_google_drive_file_info
...
def get_github_assets(repo='ia_ultralytics/assets', version='latest', retry=False):
...
def attempt_download_asset(file, repo='ia_ultralytics/assets', release='v0.0.0'):
    """Attempt file download from GitHub release assets if not found locally. release = 'latest', 'v6.2', etc."""
    from ia_ultralytics.utils import SETTINGS  # scoped for circular import
