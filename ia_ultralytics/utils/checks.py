...
from ia_ultralytics.utils import (ASSETS, AUTOINSTALL, LINUX, LOGGER, ONLINE, ROOT, USER_CONFIG_DIR, ThreadingLocked,
...
def check_latest_pypi_version(package_name='ia_ultralytics'):
    """
...
def check_pip_update_available():
    """
    Checks if a new version of the ia_ultralytics package is available on PyPI.
    ...
    if ONLINE and is_pip_package():
        with contextlib.suppress(Exception):
            ...
            if pkg.parse_version(__version__) < pkg.parse_version(latest):  # update is available
                LOGGER.info(f'New https://pypi.org/project/ia_ultralytics/{latest} available 😃 '
                            f"Update with 'pip install -U ia_ultralytics'")
                return True
    return False
...
def check_font(font='Arial.ttf'):
    ...
    url = f'https://ia_ultralytics.com/assets/{name}'
...
def check_requirements(requirements=ROOT.parent / 'requirements.txt', exclude=(), install=True, cmds=''):
    ...
    Example:
        ```python
        from ia_ultralytics.utils.checks import check_requirements

        # Check a requirements.txt file
        check_requirements('path/to/requirements.txt')

        # Check a single package
        check_requirements('ia_ultralytics>=8.0.0')

        # Check multiple packages
        check_requirements(['numpy', 'ia_ultralytics>=8.0.0'])
        ```
    """
    ...
    if s:
        if install and AUTOINSTALL:  # check environment variable
            ...
            LOGGER.info(f"{prefix} ia_ultralytics requirement{'s' * (n > 1)} {pkgs} not found, attempting AutoUpdate...")
            try:
                t = time.time()
        ...
        if 'u.yaml' in file:
            ...
            if file != original_file and verbose:
                LOGGER.info(
                    ...
                    f'trained with https://github.com/ia_ultralytics/ia_ultralytics and feature improved performance vs '
                    f'standard YOLOv5 models trained with https://github.com/ia_ultralytics/yolov5.\n')
    return file
...
def check_yolo(verbose=True, device=''):
    ...
    from ia_ultralytics.utils.torch_utils import select_device
...
def check_amp(model):
    ...
    Example:
        ...
        from ia_ultralytics.utils.checks import check_amp
    ...
    try:
        ...
        LOGGER.warning(
            f'{prefix}checks skipped ⚠️. Unable to load YOLOv8n due to possible ia_ultralytics package modifications. {warning_msg}'
        )
...
def print_args(args: Optional[dict] = None, show_file=True, show_func=False):
    ...
    def strip_auth(v):
        """Clean longer ia_ultralytics HUB URLs by stripping potential authentication information."""
