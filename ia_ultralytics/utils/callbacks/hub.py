...
from ia_ultralytics.hub.utils import HUB_WEB_ROOT, PREFIX, events
...
def on_fit_epoch_end(trainer):
    ...
    if session:
        ...
        if trainer.epoch == 0:
            from ia_ultralytics.utils.torch_utils import model_info_for_loggers
...
def on_model_save(trainer):
    """Saves checkpoints to ia_ultralytics HUB with rate limiting."""
    session = getattr(trainer, 'hub_session', None)
    if session:
...
def on_train_end(trainer):
    """Upload final model and metrics to ia_ultralytics HUB at the end of training."""
    session = getattr(trainer, 'hub_session', None)
    if session:
