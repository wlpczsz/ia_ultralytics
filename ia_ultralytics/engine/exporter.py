...
Requirements:
    $ pip install "ia_ultralytics[export]"
...
from ia_ultralytics.cfg import get_cfg
from ia_ultralytics.data.dataset import YOLODataset
from ia_ultralytics.data.utils import check_det_dataset
from ia_ultralytics.nn.autobackend import check_class_names
from ia_ultralytics.nn.modules import C2f, Detect, RTDETRDecoder
from ia_ultralytics.nn.tasks import DetectionModel, SegmentationModel
from ia_ultralytics.utils import (ARM64, DEFAULT_CFG, LINUX, LOGGER, MACOS, ROOT, WINDOWS, __version__, callbacks,
                               colorstr, get_default_args, yaml_save)
from ia_ultralytics.utils.checks import check_imgsz, check_requirements, check_version
from ia_ultralytics.utils.downloads import attempt_download_asset, get_github_assets
from ia_ultralytics.utils.files import file_size, spaces_in_path
from ia_ultralytics.utils.ops import Profile
from ia_ultralytics.utils.torch_utils import get_latest_opset, select_device, smart_inference_mode
...
class Exporter:
    ...
    def __call__(self, model=None):
        ...
        description = f'ia_ultralytics {self.pretty_name} model {f"trained on {data}" if data else ""}'
        self.metadata = {
            'description': description,
            'author': 'ia_ultralytics',
            'license': 'AGPL-3.0 https://ia_ultralytics.com/license',
        ...
        if any(f):
            ...
            LOGGER.info(f'\nExport complete ({time.time() - t:.1f}s)'
                        ...
                        f'\nPredict:         ia_yolo predict task={model.task} model={f} imgsz={imgsz} {q} {predict_data}'
                        f'\nValidate:        ia_yolo val task={model.task} model={f} imgsz={imgsz} data={data} {q} {s}'
    ...
    def export_edgetpu(self, tflite_model='', prefix=colorstr('Edge TPU:')):
        ...
        LOGGER.warning(f'{prefix} WARNING ⚠️ Edge TPU known bug https://github.com/ia_ultralytics/ia_ultralytics/issues/1185')
...
class IOSDetectModel(torch.nn.Module):
    """Wrap an ia_ultralytics YOLO model for Apple iOS CoreML export."""
