...
from ia_ultralytics.data.utils import IMG_FORMATS, VID_FORMATS
from ia_ultralytics.utils import LOGGER, is_colab, is_kaggle, ops
...
class LoadImages:
    ...
    def __init__(self, path, imgsz=640, vid_stride=1):
        ...
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
            a = str(Path(p).absolute())  # do not use .resolve() https://github.com/ia_ultralytics/ia_ultralytics/issues/2912
            if '*' in a:
...
def autocast_list(source):
    ...
    for im in source:
        if isinstance(im, (str, Path)):  # filename or uri
            ...
            raise TypeError(f'type {type(im).__name__} is not a supported ia_ultralytics prediction source type. \n'
                            f'See https://docs.ia_ultralytics.com/modes/predict for supported source types.')
